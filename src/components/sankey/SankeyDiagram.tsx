...
const CustomSankeyNode = (props: any) => {
  ...
  // Guard against undefined payload or essential layout properties
  if (typeof x === 'undefined' || typeof y === 'undefined' || typeof width === 'undefined' || typeof height === 'undefined' || !payload) {
    return null; 
  }
...
const CustomNodeLabel = (props: any) => {
  ...
  // Guard against undefined payload, essential layout properties, or missing displayName
  if (typeof x === 'undefined' || typeof y === 'undefined' || typeof width === 'undefined' || typeof height === 'undefined' || !payload || typeof payload.displayName !== 'string') {
    return null;
  }
...
export function SankeyDiagram({ data }: SankeyDiagramProps) {
  if (data && data.nodes.length > 0 && data.links.length > 0) {
    ...
    // Additional check to ensure nodes referenced in links actually exist in the prepared nodes list for Recharts
    const rechartsNodeNames = new Set(sankeyChartData.nodes.map(n => n.name));
    const validRechartsLinks = sankeyChartData.links.filter(
        link => rechartsNodeNames.has(link.source) && rechartsNodeNames.has(link.target)
    );

    if (validRechartsLinks.length === 0 && sankeyChartData.links.length > 0) {
        // This case means links were present but became invalid after node mapping,
        // which ideally shouldn't happen with current logic but is a safeguard.
        return (
            <div className="h-full w-full flex flex-col items-start text-left bg-muted/20 rounded-lg p-4">
            <h2 className="text-xl font-semibold mb-1">Live Transaction Flow</h2>
            <p className="text-sm text-muted-foreground mb-4">
                Error preparing data for Sankey Diagram. Link integrity issue.
            </p>
            <div className="flex-grow w-full flex flex-col items-center justify-center text-center">
                <Image 
                    src="https://placehold.co/800x400.png?text=Data+Error" 
                ...
                />
                <p className="mt-4 text-muted-foreground">Could not render flow data due to inconsistencies.</p>
            </div>
            </div>
        );
    }
    ...
    return (
      <div className="h-full w-full flex flex-col items-start text-left bg-muted/20 rounded-lg p-1">
        ...
        <p className="text-xs text-muted-foreground mb-1 px-3">
          Visualizing transactions. Nodes: {sankeyChartData.nodes.length}, Links: {validRechartsLinks.length}.
        </p>
        
        <ResponsiveContainer width="100%" height="100%" className="flex-grow min-h-[300px]">
          <Sankey
            data={{nodes: sankeyChartData.nodes, links: validRechartsLinks}} // Use validRechartsLinks
            ...
            <Tooltip 
              ...
              formatter={(value: any, name: any, props: any) => {
                if (props.payload && props.payload.source && props.payload.target) {
                  const sourceNode = sankeyChartData.nodes.find(n => n.name === props.payload.source.name);
                  const targetNode = sankeyChartData.nodes.find(n => n.name === props.payload.target.name);
